...
//interactions between the pool balls, cues stick, and pockets.
...
public class PoolBoard extends Observable implements Serializable {
	...
	public PoolBoard() {
		...
		setUpBalls();
		cueStick = new CueStick(balls[balls.length-1]);//cue ball
	...
	private void setUpBalls() {
		...
		for (int k = 0; k < balls.length; k++) {
			if (k == 10) { // WHITE
				balls[k] = new Ball(2, colors[9], -1, -1);
			}
			else if (k == 8) { // BLACK
				balls[k] = new Ball(3, colors[10], 8, 8);
			}
			else { // COLORED
				balls[k] = new Ball(1, colors[k], k+1, k+1);
			}
		}
	...
	public void rackBalls(Ball[] balls) {
		...
		// BLACK
		balls[8].setCenter(threeQuartersLength + 2 * incrementX, centerY);
		// COLORED
	...
	private void checkPockets() {//pocket ball
